...
Board.prototype.ChangeTally = function(col, change)
{
	...
	if (count != 0) {
	    tallyhtml += '<tr><td class="count">' + count + '</td><td>x</td><td class="paletteitem"' + this.palette.ColourNameHTML(i) + '</td></tr>\n';
	}
    }
...
{
    var moves_up = new Array;
    var moves_down = new Array;
    var moves_left = new Array;
    var moves_right = new Array;
    for (var y = 0; y < hgt; ++y)
        for (var x = 0; x < wid; ++x)
	{
	    var num = this.points.length;
	    if (y + 1 < hgt) {
		moves_up[num] = num + wid;
	    }
	    if (y > 0) {
		moves_down[num] = num - wid;
	    }
	    if (x + 1 < wid) {
		moves_left[num] = num + 1;
	    }
	    if (x > 0) {
		moves_right[num] = num - 1;
	    }
...
Toolbox.prototype.SetTool = function(tool)
{
	...
	toolelt.src = this.tool + "tool.jpg";
    }
    this.tool = tool;
    if (this.tool != null) {
	var toolelt = document.getElementById("tool_" + this.tool);
	toolelt.src = this.tool + "toolselected.jpg";
    }
}
